...
class PVG_Meta_Box {
  ...
  public function show() {
    ...
    wp_nonce_field( basename(__FILE__), 'PVG_Meta_Box_nonce' );
      ...
      if ($meta && count($meta) > 0 && is_array($meta) ){
         foreach ($meta as $me){
           ...
           if ( in_array( $field['fields'][0]['type'], array('image','file') ) )
            $mmm = $c +1 ;
           echo '<div class="widget-top" style="font-size:18px; padding:8px 0px 0px 8px;">'.'Video '.$mmm.'<br/><table class="repeater-table"  style="width:100%; display: none;">';
           if ($field['inline']){
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function save( $post_id ) {
    ...
    || ( ! check_admin_referer( basename( __FILE__ ), 'PVG_Meta_Box_nonce') )    // Check nonce - Security
    ...
    foreach ( $this->_fields as $field ) {
      ...
      // Validate meta value
      if ( class_exists( 'PVG_Meta_Box_Validate' ) && method_exists( 'PVG_Meta_Box_Validate', $field['validate_func'] ) ) {
        $new = call_user_func( array( 'PVG_Meta_Box_Validate', $field['validate_func'] ), $new );
      }
